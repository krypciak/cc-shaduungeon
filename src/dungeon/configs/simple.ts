...
export class DungeonConfigSimpleFactory implements DungeonConfigFactory {
    ...
    getSbBuilders(multi: number, exclusive: boolean = true): MapBuilderArrayGenerate {
        ...
        DirUtil.forEachUniqueDir2((d1, d2) => {
            ...
            for (let i = 0; i < multi; i++) { _sb.arr.push(b) }
        })
        this.bPool.push(_sb)
        return _sb
    }
    ...
    getDbBuilders(exitCount: number, multi: number, exclusive: boolean = true): MapBuilderArrayGenerate {
        if (exitCount < 1 || exitCount > 3) { throw new Error('invalid exit count') }
        const _db: MapBuilderArrayGenerate = { arr: [], randomize: true, index: this.bPool.length }
        
        ;(exitCount == 1 ? DirUtil.forEachUniqueDir2 :
        exitCount == 2 ? DirUtil.forEachUniqueDir3 :
        exitCount == 3 ? DirUtil.forEachUniqueDir4 : () => { throw new Error() })
        ((...arr: Dir[]) => {
            const b = Object.assign(new SimpleMultipleExitMapBuilder(this.areaInfo, arr[0], arr[1], arr[2], arr[3]), { exclusive })
            for (let i = 0; i < multi; i++) { _db.arr.push(b) }
        })
        this.bPool.push(_db)
        return _db
    }
    ...
    problematic(): DungeonGenerateConfig { /* fixed :) */
        const _sb1: MapBuilderArrayGenerate = { arr: [
        ...
        ], randomize: false, index: this.bPool.length }; this.bPool.push(_sb1)

        const _sb2: MapBuilderArrayGenerate = { arr: [
            Object.assign(Object.assign(new SimpleSingleTunnelEndMapBuilder(this.areaInfo, Dir.WEST, Dir.NORTH), { exclusive: true })),
            Object.assign(Object.assign(new SimpleSingleTunnelEndMapBuilder(this.areaInfo, Dir.SOUTH, Dir.NORTH), { exclusive: true })),
        ], randomize: false, index: this.bPool.length }; this.bPool.push(_sb2)

        const _sb3: MapBuilderArrayGenerate = { arr: [
            Object.assign(Object.assign(new SimpleSingleTunnelEndMapBuilder(this.areaInfo, Dir.EAST, Dir.NORTH), { exclusive: true })),
        ], randomize: false, index: this.bPool.length }; this.bPool.push(_sb3)

        const _db1: MapBuilderArrayGenerate = { arr: [
            Object.assign(new SimpleMultipleExitMapBuilder(this.areaInfo, Dir.SOUTH, Dir.NORTH, Dir.EAST), { exclusive: true })
        ], randomize: false, index: this.bPool.length }; this.bPool.push(_db1)

        const _db2: MapBuilderArrayGenerate = { arr: [
            Object.assign(new SimpleMultipleExitMapBuilder(this.areaInfo, Dir.SOUTH, Dir.WEST), { exclusive: true })
        ], randomize: false, index: this.bPool.length }; this.bPool.push(_db2)
